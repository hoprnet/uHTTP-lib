...
const RelayNodesCompatVersions = ['2.1.0-rc.4-pr.6235'];
...
export type NodePair = {
    ...
    log: ReturnType<typeof RoutingUtils.logger>;
...
export function prettyPrint(np: NodePair): string {
    ...
    const exStrs = Array.from(np.exitDatas).map(([id, d]) => {
        ...
        const str = prettyOngoingNumbers(np, o, lats.length, tot, Utils.average(lats));
        const nId = Utils.shortPeerId(id);
    ...
    const segStr = prettyOngoingNumbers(
        ...
        segLats.length,
        segTotal,
        Utils.average(segLats),
    );
    const mesLat = Utils.average(np.entryData.fetchMessagesLatencies);
